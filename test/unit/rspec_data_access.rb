    ...
    describe '#updateStock' do
        ...
        context "book is new" do
          it "should add it to database but leave remote cache unchanged" do
            ...
            expect(result).to eql({isbn: @book1.isbn, qty: @book1.quantity })  
         end
       end 
       ...
       context "adding to stock of existing book on stock" do
          context "when it is not in the remote cache" do
             it "should leave the remote cache unchanged" do
               expect(@sqlite_database).to receive(:updateStock).with(@book1).
                     and_return( (@book1.quantity + 5) )
               expect(@dalli_client).to receive(:get).with("v_#{@book1.isbn}" ).
                   and_return(nil)
               result = @data_access.updateStock(@book1) 
               expect(result).to eql({ isbn: @book1.isbn, qty: (@book1.quantity + 5) }) 
             end
          end
          ...
          context "when it is in the remote cache" do
             it "should update the remote cache unchanged" do
               ...
               expect(@dalli_client).to receive(:set).with("v_#{@book1.isbn}",3)
               @book1.quantity += 5
               expect(@dalli_client).to receive(:set).with("#{@book1.isbn}_3", 
                       @book1.to_cache)
               result = @data_access.updateStock(@book1) 
               expect(result).to eql ({ isbn: @book1.isbn, qty: @book1.quantity }) 
             end
